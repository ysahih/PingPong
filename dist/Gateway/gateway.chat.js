...
const exception_filter_1 = require("./ExceptionFilter/exception.filter");
const geteway_service_1 = require("./geteway.service");
const jwt = require("jsonwebtoken");
const user_service_1 = require("../user/user.service");
const gateway_gameclasses_1 = require("./gateway.gameclasses");
let serverGateway = serverGateway_1 = class serverGateway {
    constructor(_users, _rooms, _prisma, FriendsService) {
        this._users = _users;
        this._rooms = _rooms;
        this._prisma = _prisma;
        this.FriendsService = FriendsService;
        this._server = new socket_io_1.Server();
        this.logger = new common_1.Logger(serverGateway_1.name);
        this.gameRooms = new gateway_gameclasses_1.datagame();
    }
    async afterInit() {
    ...
    async handleConnection(client) {
        try {
            ...
            allSockets.forEach((sokcetId) => this._server.to(sokcetId).emit("online", { id: user.id }));
    ...
    async handleDisconnect(client) {
        ...
        if (id < 0)
            return;
        const allSockets = this._users.getAllSocketsIds();
        allSockets.forEach((sokcetId) => this._server.to(sokcetId).emit("offline", { id: id }));
    ...
    async handleDirectMessage(client, payload) {
        ...
        if (!isExist) {
            console.log(...oo_oo(`2342311917_139_6_139_50_4`, "Conversation does not Exist !"));
            const newConv = await this._prisma.createConversation(payload);
            this._users.addNewConversation(payload, newConv.id);
        }
        else {
            console.log(...oo_oo(`2342311917_145_6_145_42_4`, "Conversation Exists !"));
            await this._prisma.updateConversation(isExist.id, payload);
        }
        const toUser = this._users.getUserById(payload.to);
        if (toUser) {
            const lastMessage = await this._prisma.uniqueConvo(payload.from, payload.to);
            toUser.socketId.forEach((socktId) => this._server.to(socktId).emit("newConvo", lastMessage));
        }
    }
    async handleCreateRoom(client, payload) {
        try {
            ...
            console.log(...oo_oo(`2342311917_178_6_178_59_4`, this._users.getUserById(payload.ownerId)));
        }
        catch (e) {
            if (e instanceof client_1.Prisma.PrismaClientKnownRequestError) {
                console.log(...oo_oo(`2342311917_186_8_186_36_4`, 'Errrrooooor !'));
                if (e.code === "P2002")
                    this._server
                        .to(client.id)
    ...
    async handleJoinRoom(client, payload) {
        ...
        try {
            if (foundedRoom) {
                ...
                console.log(...oo_oo(`2342311917_219_8_219_60_4`, this._users.getUserById(payload.userId)));
            }
            else {
                this._server
                    .to(client.id)
            ...
            if (e.code === "P2002")
                ...
                this._server
                    .to(client.id)
                    .emit("error", `User with ${payload.userId}:ID can't join the room !`);
        }
    }
    handleUserStatusInRoom(payload) {
        console.log(...oo_oo(`2342311917_248_4_248_24_4`, payload));
    }
    async handleNewFriend(client, Payload) {
        const targetFriend = await this.FriendsService.sendFriendRequest(Payload.userId, Payload.id);
        if (targetFriend) {
            ...
            if (Sockets) {
                Sockets.socketId.forEach((socktId) => {
                    this._server.to(socktId).emit("NewInvit", newInvit);
                    notifications && this._server.to(socktId).emit("Notification", notifications);
                });
                console.log(...oo_oo(`2342311917_283_2_283_54_4`, " newInvet from :", targetFriend.sender));
            }
        }
    }
    async handleDenyFriend(Payload) {
        const targetFriend = await this.FriendsService.deleteFriendRequest(Payload.userId, Payload.id);
        console.log(...oo_oo(`2342311917_294_4_294_68_4`, "deny : ", targetFriend, Payload.id, Payload.userId));
        if (targetFriend !== null) {
            const SocketsTarget = this._users.getUserById(Payload.userId);
            if (SocketsTarget) {
                SocketsTarget.socketId.forEach((socktId) => {
    ...
    async handleDeleteFriend(Payload) {
        const targetFriend = await this.FriendsService.deleteFriendRequest(Payload.id, Payload.userId);
        console.log(...oo_oo(`2342311917_312_4_312_68_4`, "deny : ", targetFriend, Payload.id, Payload.userId));
        if (targetFriend !== null) {
    ...
    async handleAcceptFriend(Payload) {
        const targetFriend = await this.FriendsService.acceptFriendRequest(Payload.userId, Payload.id);
        if (targetFriend !== null) {
            ...
            if (client) {
                client.socketId.forEach((socktId) => {
                    this._server.to(socktId).emit("NewFriend", targetFriend.reciever);
                });
            }
        }
    }
    async handleNewBlocked(Payload) {
        const targetFriend = await this.FriendsService.blockFriendRequest(Payload.userId, Payload.id);
        console.log(...oo_oo(`2342311917_357_2_357_38_4`, "blocked", targetFriend));
        if (!targetFriend)
            return;
        const SocketsTarget = this._users.getUserById(Payload.id);
        if (SocketsTarget) {
    ...
    async handleUnBlocked(Payload) {
        const targetFriend = await this.FriendsService.unblockFriendRequest(Payload.userId, Payload.id);
        if (targetFriend !== null) {
            console.log(...oo_oo(`2342311917_386_3_386_41_4`, "unblocked", targetFriend));
            const client = this._users.getUserById(Payload.userId);
            if (client) {
                client.socketId.forEach((socktId) => {
    ...
    async deleteRoom(roomId) {
        ...
        console.log(...oo_oo(`2342311917_406_4_406_42_4`, `Room ${roomId} deleted.`));
    }
    async handleJoinRome(client, lodingdata) {
        let user;
        try {
        ...
        console.log(...oo_oo(`2342311917_422_4_422_18_4`, 1));
        if (!curentroom) {
            curentroom = this.gameRooms.findEmptyRoom(lodingdata.type, lodingdata.userid, lodingdata.mode);
            try {
                if (!curentroom) {
                    console.log(...oo_oo(`2342311917_429_6_429_20_4`, 2));
                    ...
                    console.log(...oo_oo(`2342311917_435_6_435_20_4`, 3));
            ...
            console.log(...oo_oo(`2342311917_446_5_446_19_4`, 4));
    ...
    async gameInvitation(client, mydata) {
        let user;
        try {
    ...
    async SendGameInvite(client, mydata) {
...
], serverGateway.prototype, "_server", void 0);
__decorate([
    __param(0, (0, websockets_1.ConnectedSocket)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [socket_io_1.Socket]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "handleDisconnect", null);
__decorate([
    (0, common_1.UseFilters)(exception_filter_1.ExceptionHandler),
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [socket_io_1.Socket,
        gateway_interface_1.MessageDTO]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "handleDirectMessage", null);
__decorate([
    (0, common_1.UseFilters)(exception_filter_1.ExceptionHandler),
    (0, common_1.UsePipes)(common_1.ValidationPipe),
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [socket_io_1.Socket,
        gateway_interface_1.CreateRoom]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "handleCreateRoom", null);
__decorate([
    (0, common_1.UseFilters)(exception_filter_1.ExceptionHandler),
    (0, common_1.UsePipes)(common_1.ValidationPipe),
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [socket_io_1.Socket,
        gateway_interface_1.JoinRoomDTO]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "handleJoinRoom", null);
__decorate([
    (0, common_1.UseFilters)(exception_filter_1.ExceptionHandler),
    (0, common_1.UsePipes)(common_1.ValidationPipe),
    (0, websockets_1.SubscribeMessage)('userStatusInRoom'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [gateway_interface_1.UpdateStatusRoom]),
    __metadata("design:returntype", void 0)
], serverGateway.prototype, "handleUserStatusInRoom", null);
__decorate([
    (0, websockets_1.SubscribeMessage)("NewInvit"),
    __param(0, (0, websockets_1.ConnectedSocket)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [socket_io_1.Socket, Object]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "handleNewFriend", null);
__decorate([
    (0, websockets_1.SubscribeMessage)("DenyFriend"),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "handleDenyFriend", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "handleDeleteFriend", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "handleAcceptFriend", null);
__decorate([
    ...
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "handleNewBlocked", null);
__decorate([
    (0, websockets_1.SubscribeMessage)("UnBlocked"),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "handleUnBlocked", null);
__decorate([
    (0, websockets_1.SubscribeMessage)('RandomGameroom'),
    __param(0, (0, websockets_1.ConnectedSocket)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [socket_io_1.Socket, Object]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "handleJoinRome", null);
__decorate([
    ...
    __metadata("design:paramtypes", [socket_io_1.Socket, Object]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "Game", null);
__decorate([
    (0, websockets_1.SubscribeMessage)('gameInvitation'),
    __param(0, (0, websockets_1.ConnectedSocket)()),
    __param(1, (0, websockets_1.MessageBody)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [socket_io_1.Socket, Object]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "gameInvitation", null);
__decorate([
    ...
    __metadata("design:paramtypes", [socket_io_1.Socket, Object]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "endGame", null);
__decorate([
    ...
    __metadata("design:paramtypes", [socket_io_1.Socket, Object]),
    __metadata("design:returntype", Promise)
], serverGateway.prototype, "SendGameInvite", null);
exports.serverGateway = serverGateway = serverGateway_1 = __decorate([
    ...
    __metadata("design:paramtypes", [user_class_1.UsersServices,
        room_class_1.RoomsServices,
        geteway_service_1.GatewayService,
        user_service_1.FriendsService])
], serverGateway);
;
function oo_cm() { try {
